...
import { addLocaleData, getLocaleData } from './LocaleDataStore'; // For historical reasons, "approximate" is the default style.

import defaultStyle from './style/approximate';
import getStyleByName from './style/getStyleByName'; // Valid time units.
  ...
  _createClass(TimeAgo, [{
    ...
    value: function format(input) {
      ...
      var _getDateAndTimeBeingF = getDateAndTimeBeingFormatted(input),
          ...
          time = _getDateAndTimeBeingF.time; // Get locale messages for this type of labels.
      ...
      var secondsPassed = (now - time) / 1000; // in seconds
      ...
      style.gradation || style.steps || defaultStyle.gradation, secondsPassed, {
        now: now,
        units: units,
        future: future,
        getNextStep: true
      }),
          _getStep2 = _slicedToArray(_getStep, 2),
          step = _getStep2[0],
          nextStep = _getStep2[1];

      var formattedDate = this.formatDateForStep(date || time, step, secondsPassed, {
        labels: labels,
      ...
      if (options.getTimeToNextUpdate) {
        var timeToNextUpdate = step && getTimeToNextUpdate(date || time, step, {
          nextStep: nextStep,
          now: now,
          future: future
        });
    ...
    value: function formatDateForStep(dateOrTimestamp, step, secondsPassed, _ref2) {
      var _this = this;
      ...
      var valueForFormatting = -1 * Math.sign(secondsPassed) * Math.round(amount); // By default, this library formats a `0` in "past" mode,
...
TimeAgo.addLabels = function (locale, name, labels) {
  ...
  if (!localeData) {
    throw new Error("[javascript-time-ago] No data for locale \"".concat(locale, "\""));
  }
